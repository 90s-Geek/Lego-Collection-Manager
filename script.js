...
async function loadViewPreference() {
    const { data } = await db.from('user_preferences').select('view_mode').eq('id', 'default').single();
    if (data) currentView = data.view_mode;
}

async function setView(mode) {
    currentView = mode;
    const { error } = await db.from('user_preferences').update({ view_mode: mode, updated_at: new Date() }).eq('id', 'default');
    if (error) console.error('Failed to save view preference:', error.message);
