...
async function loadViewPreference() {
    // Each page has its own preference row: id = 'collection' or 'wantlist'
    const pageId = isWantlistPage() ? 'wantlist' : 'collection';
    const { data, error } = await db
        .from('user_preferences')
        .select('view_mode')
        .eq('id', pageId)
        .single();
    // Fall back to 'list' if row missing or error
    currentView = (data && data.view_mode) ? data.view_mode : 'list';
}

async function setView(mode) {
    currentView = mode;
    const pageId = isWantlistPage() ? 'wantlist' : 'collection';
    // Upsert so the row is created automatically if it doesn't exist yet
    await db.from('user_preferences').upsert(
        { id: pageId, view_mode: mode, updated_at: new Date() },
        { onConflict: 'id' }
    );
