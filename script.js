...
async function loadViewPreference() {
    ...
    const { data } = await db.from('user_preferences').select(col).eq('id', 'default').single();
    if (data) currentView = data[col] || 'list';
}

async function setView(mode) {
    currentView = mode;
    const col = isWantlistPage() ? 'view_mode_wantlist' : 'view_mode';
    await db.from('user_preferences').update({ [col]: mode, updated_at: new Date() }).eq('id', 'default');
