...
// Variable 'db' avoids the 'supabase has already been declared' error
const db = supabase.createClient(SB_URL, SB_KEY);
let currentSet = null;

window.onload = () => {
    // Only fetch data if we are on the collection page
...
async function searchLego() {
    ...
    try {
        const response = await fetch(url, { headers: { 'Authorization': `key ${REBRICKABLE_KEY}` } });
